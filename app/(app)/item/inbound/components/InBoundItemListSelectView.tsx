...
const unselectedStyle = "border-gray-200 hover:border-blue-400 hover:shadow-lg";

function ItemThumbnail({ imageUrl, name }: { imageUrl?: string | null; name: string }) {
    if (imageUrl) {
        return <img src={imageUrl} alt={name} className="w-full h-32 object-cover rounded-lg mb-3" />;
    }
    return (
...
export default function InBoundItemListSelectView({ items, fields, onAddItem, onClose }: Props) {
    return (
